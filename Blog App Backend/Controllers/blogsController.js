...
const searchbyid = (req,res)=>{
  ...
  find = data.find(data=>data.id == id);
  if (find){
  res.send(find);}
  else{
    res.send('blogs do not exist');
  }}
const updatePost = (req, res) => {
    ...
    const updatedData = Data.map(blog => (blog.id === id ? { ...blog, title, content, author } : blog));
...
module.exports = {getAllPostsController,searchbyid,updatePost,createPost,deletepost}